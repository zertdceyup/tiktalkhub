...
export const initializeDatabase = async () => {
  try {
    ...
    const settingExists = db.prepare('SELECT COUNT(*) as count FROM admin_settings WHERE key = ?');
    const insertSetting = db.prepare('INSERT INTO admin_settings (key, value, type, category, description) VALUES (?, ?, ?, ?, ?)');

    for (const setting of defaultSettings) {
      if (settingExists.get(setting.key).count === 0) {
        insertSetting.run(setting.key, setting.value, setting.type || 'string', setting.category, setting.description);
      }
    }
    ...
    const toolExists = db.prepare('SELECT COUNT(*) as count FROM tools WHERE slug = ?');
    const insertTool = db.prepare('INSERT INTO tools (name, slug, category, description, icon) VALUES (?, ?, ?, ?, ?)');

    for (const tool of defaultTools) {
      if (toolExists.get(tool.slug).count === 0) {
        insertTool.run(tool.name, tool.slug, tool.category, tool.description, tool.icon);
      }
    }
